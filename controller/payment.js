...
exports.initailizePayment = async (req, res) => {
  try {
    ...
    const member = group.members.find((member) => member.toString() === signUpId);

    if (!member) {
      return res.status(404).json({
        message: "signUp is not a member of this group",
      });
    }
...
exports.getAllPaymentBysignUp = async (req, res) => {
  try {
    const signUpId = req.signUp.id;
    const signUp = await signUpModel.findById(signUpId);
    if (!signUp) {
